...
<title>Chrono — Powered by Resend</title>
...
<div class="main">
  <div>
    ...
    <p class="hero-sub">Powered by Resend — built for developers, trusted by inboxes. Follow these steps once, then manage everything on the right.</p>
    ...
    <div class="steps">
      <div class="step">
        ...
        <div class="step-content">
          ...
          <p>Go to <code>resend.com</code> and sign up. 3,000 emails/month and 100/day free — no credit card needed.</p>
        </div>
      </div>
      <div class="step">
        ...
        <div class="step-content">
          ...
          <p>Resend → Domains → Add Domain. Resend gives you DNS records (SPF, DKIM) to paste into your registrar. This is what keeps emails out of spam. ~5 min setup.</p>
        </div>
      </div>
      <div class="step">
        ...
        <div class="step-content">
          ...
          <p>Resend → API Keys → Create API Key. Set permission to <code>Sending access</code> only. Paste it in the Config tab →</p>
        </div>
      </div>
      <div class="step">
    ...
    <div class="resend-card">
      <h3>⚡ Why Resend?</h3>
      <div class="ri"><div class="ri-a">▸</div><div>3,000 emails/month free — no credit card required</div></div>
      <div class="ri"><div class="ri-a">▸</div><div>Built on AWS SES infrastructure — rock-solid uptime, zero recent incidents</div></div>
      <div class="ri"><div class="ri-a">▸</div><div>One-step domain verification auto-configures SPF + DKIM</div></div>
      <div class="ri"><div class="ri-a">▸</div><div>Clean dashboard with delivery logs, open tracking & webhooks</div></div>
      <div class="ri"><div class="ri-a">▸</div><div>Modern REST API — no legacy SDK overhead</div></div>
    </div>
  </div>
  ...
  <div>
    <div class="app-window">
      <div class="wbar">
        ...
        <div class="wtitle">CHRONO — RESEND EDITION</div>
      ...
      <div class="app-inner">
        ...
        <div class="tab-panel active" id="tab-events">
          ...
          <div class="events-list" id="eventsList">
            <div class="empty"><div class="empty-icon">🗓</div>No events yet — add your first one.</div>
          </div>
        </div>

        <div class="tab-panel" id="tab-config">
          <div class="cfg-section">
            <div class="cfg-title">✉️ Resend Configuration</div>
            <div class="frow">
              <div class="f">
          ...
          <button class="btn-primary" onclick="testEmail()">📨 Send Test Email</button>
        </div>

        <div class="tab-panel" id="tab-code">
          <p class="code-note">Node.js backend using the official <strong style="color:var(--text)">Resend SDK</strong>. Deploy to Railway or Render — set your env vars and you're live.</p>
          <div class="code-wrap">
            <button class="btn-copy" onclick="copyCode()">COPY</button>
            <div class="code-block" id="codeBlock"><span class="cm">// server.js — Chrono x Resend</span>
...
<span class="cm">// Runs every hour — checks for events needing a reminder</span>
  ...
  <span class="ky">for</span> (<span class="ky">const</span> <span class="vr">ev</span> <span class="ky">of</span> <span class="vr">events</span>) {
    ...
    <span class="ky">if</span> (<span class="vr">hoursLeft</span> > <span class="nm">0</span> && <span class="vr">hoursLeft</span> <= <span class="vr">ev</span>.reminderHours) {
      <span class="ky">await</span> <span class="vr">resend</span>.emails.<span class="fn">send</span>({
        ...
        subject: `⏰ ${<span class="vr">ev</span>.title} — coming up soon`,
      ...
      <span class="vr">console</span>.<span class="fn">log</span>(`✓ Reminded: ${<span class="vr">ev</span>.title}`);
    }
  }
...
<span class="vr">app</span>.<span class="fn">listen</span>(<span class="nm">3000</span>, () =>
  <span class="vr">console</span>.<span class="fn">log</span>(<span class="st">'🟢 Chrono running on :3000'</span>));
</div>
          </div>
          <div class="env-box">
            <span style="color:var(--accent)">ENV VARS TO SET</span><br>
            RESEND_API_KEY=re_xxxxxxxxxxxx<br>
            TO_EMAIL=you@gmail.com<br>
            FROM_EMAIL=alerts@yourdomain.com<br>
  ...
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div class="mf">
        <label>Color</label>
        <select id="newColor">
          <option value="#00e5a0">🟢 Mint</option>
...
<script>
let events = JSON.parse(localStorage.getItem('chronoEventsR') || '[]');
...
function addEvent() {
  ...
  events.push(ev);
  localStorage.setItem('chronoEventsR', JSON.stringify(events));
  renderEvents();
  closeModal();
  showToast(`✅ "${title}" saved — reminder set ${ev.reminderHours}h before.`, 'success');
}
...
function renderEvents() {
  ...
  if (!events.length) {
    list.innerHTML = '<div class="empty"><div class="empty-icon">🗓</div>No events yet — add your first one.</div>';
    return;
  }
  ...
  list.innerHTML = sorted.map(ev => {
    ...
    return `<div class="ev-item">
      ...
      <div class="ev-info">
        ...
        <div class="ev-meta">${d.toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric'})} · ${d.toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'})} · 🔔 ${ev.reminderHours}h${ev.notes ? ' · '+ev.notes.slice(0,28) : ''}</div>
      </div>
      <div class="ev-badge ${bc}">${badge}</div>
      <button class="btn-rm" onclick="removeEvent(${ev.id})">✕</button>
    </div>`;
  }).join('');
}
...
function testEmail() {
  ...
  if (!config.apiKey || !config.toEmail || !config.fromEmail) {
    showToast('⚠️ Fill in the Config tab first.', 'error'); return;
  }
  showToast('📨 Test triggered! Start your backend server to see it land.', 'success');
}
